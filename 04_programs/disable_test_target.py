...
def update_test_target_to_false(file_prefix):
    ...
    for filename in os.listdir(input_dir):
        if filename.startswith(file_prefix) and filename.endswith('.json'):
            ...
            try:
                ...
                if 'menus' in data:
                    update_items_recursively(data['menus'], False)
                
                with open(filepath, 'w', encoding='utf-8') as f:
                    json.dump(data, f, ensure_ascii=False, indent=4)
                print(f"'{filename}' 파일의 모든 menuLevel 항목에 대해 isTestTarget을 false로 업데이트했습니다.")
                break
...
if __name__ == "__main__":
    ...
    if len(prefix) == 3 and prefix.isdigit():
        update_test_target_to_false(prefix)
    else:
